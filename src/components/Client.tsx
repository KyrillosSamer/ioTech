...
export default function FeedBack() {
  ...
  const handlePrev = useCallback(() => setCurrentIndex(prev => (prev === 0 ? clients.length - 1 : prev - 1)), [clients.length]);
  const handleNext = useCallback(() => setCurrentIndex(prev => (prev === clients.length - 1 ? 0 : prev + 1)), [clients.length]);

  // 
  const client = clients[currentIndex] || { name: '', position: '', feedback: '', img: '/Imgs/person.png' };

  const memoizedClientText = useMemo(() => (
    <>
  ...
  return (
    ...
    >
      {clients.length === 0 ? (
        <div className="w-full h-[374px] flex items-center justify-center text-gray-700">
